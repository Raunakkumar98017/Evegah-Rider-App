...
class UnlockingScreen extends StatefulWidget {
  ...
  const UnlockingScreen({super.key, required this.vehicleId});
...
class _UnlockingScreenState extends State<UnlockingScreen> {
  final UnlockService unlockService = UnlockService();
  ...
  Future<void> startUnlockFlow() async {
    ...
    if (widget.vehicleId == "TEST123") {
      await Future.delayed(const Duration(seconds: 2)); // Fake a cool loading animation
    ...
    if (distance > 20) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Move closer to vehicle 🚗\nDistance: ${distance.toStringAsFixed(1)} meters")),
      );
      Navigator.pop(context);
      return;
    }
    ...
    if (unlocked) {
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (context) => RideStartedScreen(vehicleId: widget.vehicleId)),
      );
    }
  }
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: const Icon(Icons.lock_open, size: 70, color: Colors.green),
            ),
